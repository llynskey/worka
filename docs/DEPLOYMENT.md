...
Domains are configured in `deploy/Caddyfile` (currently `woka.site`, `www.woka.site`, and `api.woka.site`). Point DNS `A` records for all three at the VPS IPv4 address. The `api.` subdomain is what the native iOS/Android apps call; web traffic stays same-origin through `/api`.
...
Smtp__From=no-reply@woka.site
```

Restart the api container and confirm `https://woka.site/api/health` reports `"email": "configured"`. Do NOT run your own mail server on the VPS — OVH blocks port 25 by default and a fresh IP has no sending reputation, so resets would land in spam.
